...
def artist(request, **kwargs):
	...
	single_album = Album.objects.filter(artist__id=artist_id)[:1]
	genre = single_album.genre
	...
	for album in albums:
		...
		user_count = Review.objects.filter(song=song_id).filter(user__critic=None).count()
		user_total_rating = Review.objects.filter(song=song_id).filter(user__critic=None).aggregate(Sum('rating'))['rating__sum']
		...
		for song in Song.objects.filter(album=album.id):
			...
			user_song_reviews = Review.objects.filter(album=album.id).filter(user__critic=None)
			critic_song_reviews = Review.objects.filter(album=album.id).exclude(user__critic=None)
		
			user_song_count = Review.objects.filter(song=song_id).filter(user__critic=None).count()
			user_song_total_rating = Review.objects.filter(song=song_id).filter(user__critic=None).aggregate(Sum('rating'))['rating__sum']
			critic_song_count = Review.objects.filter(album=album.id).exclude(user__critic=None).count()
			critic_song_total_rating = Review.objects.filter(album=album.id).exclude(user__critic=None).aggregate(Sum('rating'))['rating__sum']


			if(critic_song_count != 0):
				song_critic_rating = int(round(critic_song_total_rating/critic_song_count))
				if(song_critic_rating>70):
					album.top_songs[i]=song
					i+=1
			else:
